...
TEXT_GENERATION_SOURCE_MODELS="OpenVINO/TinyLlama-1.1B-Chat-v1.0-int8-ov"
PORT=8002
...
for MODEL in "${MODELS[@]}"; do
  if [ ! -d models/$MODEL ]; then
    python workspace/export_model.py text_generation --source_model $MODEL --weight-format int8 --kv_cache_precision u8 --target_device GPU --config_file_path models/config_rag.json --model_repository_path models
  fi
